...
const sourcePath = 'c:\\Users\\HAZEM-\\Desktop\\Hazem\\New folder\\الموقع الشامل لكل الامتحانات\\الخريطة_الذهنية_مجتمعات.html';
const destPath = 'c:\\Users\\HAZEM-\\Desktop\\Hazem\\New folder\\الموقع الشامل لكل الامتحانات\\الخريطة_الذهنية_ذوي.html';
...
try {
    ...
    // 3. Replace Navigation Pills
    const navPillsMatch = html.match(/<div class="nav-grid-container">([\\s\\S]*?)<\/div>\\s*<!-- Controls Row -->/);
    if (navPillsMatch) {
        const newNavPills = `
...
`;
        html = html.replace(navPillsMatch[1], newNavPills.replace('<div class="nav-grid-container">', '').replace('</div>', ''));
    }

    // 4. Replace Network Grid Content
    const mainContentMatch = html.match(/<div class="network-grid">([\\s\\S]*?)<\/div>\\s*<\/div>\\s*<\/main>/);
...
`;
    html = html.replace(mainContentMatch[1], newContent);

    // 5. Clean up JS specific to the other page (card names for search)
    html = html.replace(
        /const cardNames = {\\s*'1': '📘 الاستراتيجيات',\\s*'2': '⚙️ النماذج العلاجية',\\s*'3': '🎭 التفاصيل والخطوات'\\s*};/,
        \`const cardNames = {
        '1': '📘 مفاهيم أساسية',
        '2': '⏳ التطور التاريخي',
        '3': '🧬 مسببات وأنواع',
